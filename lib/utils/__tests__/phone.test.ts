...
describe('normalizePhone', () => {
  ...
  it('T1-01: already E.164 ID number passes through', () => {
    expect(normalizePhone('+6281200090201', 'ID')).toEqual({ ok: true, e164: '+6281200090201' })
  })

  it('T1-02: Indonesian national with leading 0', () => {
    expect(normalizePhone('081200090201', 'ID')).toEqual({ ok: true, e164: '+6281200090201' })
  })

  it('T1-03: Indonesian national with leading 0 and spaces', () => {
    expect(normalizePhone('0812 0009 0201', 'ID')).toEqual({ ok: true, e164: '+6281200090201' })
  })
  ...
  it('national format without leading 0 (ID)', () => {
    expect(normalizePhone('81200090201', 'ID')).toEqual({ ok: true, e164: '+6281200090201' })
  })
  ...
  it('legacy: 11-digit ID without country code', () => {
    expect(normalizePhone('81200090101', 'ID')).toEqual({ ok: true, e164: '+6281200090101' })
  })

  it('legacy: 10-digit ID without country code', () => {
    expect(normalizePhone('8120009031', 'ID')).toEqual({ ok: true, e164: '+628120009031' })
  })

  it('legacy: 9-digit ID without country code', () => {
    expect(normalizePhone('812000904', 'ID')).toEqual({ ok: true, e164: '+62812000904' })
  })
  ...
  it('typed +62 (ID) with SG selector → trusts + prefix, returns ID number', () => {
    expect(normalizePhone('+6281200090201', 'SG')).toEqual({ ok: true, e164: '+6281200090201' })
  })
