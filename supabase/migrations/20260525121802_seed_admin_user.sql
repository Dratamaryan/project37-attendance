...
BEGIN
  ...
  FROM auth.users
  WHERE email = 'admin-example@example.test';

  IF admin_uid IS NULL THEN
    RAISE WARNING 'seed_admin_user: no auth.users row found for admin-example@example.test — skipping insert';
    RETURN;
  END IF;

  INSERT INTO app_users (id, email, full_name, role, active)
  VALUES (admin_uid, 'admin-example@example.test', 'Ryan Dratama', 'admin', true)
