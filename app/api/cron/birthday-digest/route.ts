...
export async function GET(request: Request) {
  ...
  if (!cronSecret) {
    console.error('[birthday-digest] CRON_SECRET not configured — config drift');
  ...
  try {
    ...
    const result = await runBirthdayDigest({ supabase, now: new Date() });

    // Send failures must log and skip, not throw — the digest already recorded a
    // 'degraded' system_health row; a 500 here would be a false infra-failure signal.
    if (result.status === 'send_failed') {
      console.error('[birthday-digest] Telegram send failed:', result.reason);
    }

    return Response.json({ ok: true, ...result }, { status: 200 });
  } catch (err) {
    console.error('[birthday-digest] infrastructure failure:', err);
